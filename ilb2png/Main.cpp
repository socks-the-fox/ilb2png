...
int main(int argc, char* *argv)
{
	...
	std::tr2::sys::path ilbPath = argv[1];
	std::tr2::sys::path outDir = "";

	if (argc < 3)
	{
		outDir = std::tr2::sys::current_path();
		outDir /= ilbPath.stem();
	}
	else
	{
	...
	if (!std::tr2::sys::is_regular_file(ilbPath))
	{
	...
	if (!std::tr2::sys::is_directory(outDir))
	{
		try
		{
			std::tr2::sys::create_directories(outDir);
		}
...
std::shared_ptr<Image> readType16(std::fstream &ilbFile, uint32_t imgDirectory, bool isSprite, bool isRLE, bool isTransparent)
{
	...
	std::fstream::streampos oldpos = 0;
